...
本项目在 **Windows 11** 操作系统下的 **WSL2** (**Windows Subsystem for Linux 2**) **Arch Linux** 环境中进行开发和测试。

#### 1.3.1 **核心依赖工具版本**

以下是本项目构建和版本控制所依赖的主要工具及其**精确版本**信息：

| 工具名称 | 描述 | 版本号 | 架构/目标 |
| :--- | :--- | :--- | :--- |
| **GCC** (GNU Compiler Collection) | C/C++ 编译器套件 | `15.2.1` (`20250813`) | `x86_64-pc-linux-gnu` |
| **GNU Make** | 项目自动化构建工具 | `4.4.1` | `x86_64-pc-linux-gnu` |
| **Git** | 分布式版本控制系统 | `2.51.0` | `x86_64-pc-linux-gnu` |

#### 1.3.2 **关键编译配置**

项目源代码的编译配置使用 **G++** 编译器，并遵循以下**关键编译选项**：

```shell
-std=c++17 -Wall -Wextra -g -pthread
```

**说明：**

* `-std=c++17`：启用 **C++17** 语言标准支持。
* `-Wall -Wextra`：启用所有**警告**和额外的**严格警告**，以确保代码质量。
* `-g`：在目标文件中包含**调试**信息。
* `-pthread`：支持 **POSIX Threads**（多线程）库。
